<template>
    <div>
        ...
        <b-field :label="label">
            <div>
                <b-field v-for="(field, index) in fields" :key="index">
                    <b-input :value="fields[index]" @blur="setData($event, index)" v-bind="$attrs" expanded></b-input>
        ...
        <button @click="addField" class="button is-info"><b-icon pack="fa" icon="plus" size="is-small"></b-icon> &nbsp; {{addFieldLabel}}</button>
    </div>
</template>
<script>
    export default {
