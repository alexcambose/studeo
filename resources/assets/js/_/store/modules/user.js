...
const actions = {
    ...
    updateUserProfile({ commit }, { nickname, sex, description, city, school, school_level, phone, birthday, cover_color, is_teacher, social: { facebook } }) {
        return new Promise((resolve, reject) => {
            axios.post(config.url.USER_UPDATE_PROFILE, { nickname, sex, description, city, school, school_level, phone, birthday, cover_color, is_teacher, social: { facebook } })
                .then(({ data }) => {
                    if (data.success) {
                        commit(USER_AUTH_UPDATE_DATA, data);
                        resolve();
